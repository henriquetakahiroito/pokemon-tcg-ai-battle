...
for fp in files:
    ...
    arch_seat={names.index('ShumpeiNomura')}
    ng+=1; used=False
    for step in d['steps']:
...
np.savez_compressed('_bc_data_shumpei.npz', X=X, C=C, Y=Y, G=G, vocab_size=len(vocab))
pickle.dump(vocab, open('_bc_vocab_shumpei.pkl','wb'))
print('saved _bc_data_shumpei.npz + _bc_vocab_shumpei.pkl')
